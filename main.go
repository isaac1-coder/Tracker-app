...
var (
	...
	activeCallers = make(map[string]string) // Caller -> Receiver
	...
	for {
		...
		case "add_friend":
			...
			if f, ok := users[target]; ok && currentUser != nil {
				...
				notify(currentUser, "System", "Added "+f.Nickname)
			}
		...
		case "call_request":
			...
			if len(activeCallers) > 0 {
				notify(currentUser, "System", "Lines Busy")
			} else {
		...
		case "call_accepted":
			...
			if caller, ok := users[callerPhone]; ok && currentUser != nil {
				caller.Conn.WriteJSON(map[string]string{"type": "call_connected", "with": currentUser.Nickname})
				currentUser.Conn.WriteJSON(map[string]string{"type": "call_connected", "with": caller.Nickname})
...
func sendLoc(to, about *User) {
	if to.Conn != nil {
		to.Conn.WriteJSON(map[string]interface{}{"type": "loc_update", "phone": about.Phone, "nick": about.Nickname, "lat": about.Lat, "lng": about.Lng})
	}
}
